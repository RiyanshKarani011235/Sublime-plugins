	...
	def generate_test_bench(self) : 
		...
		for signal_declaration in self.signal_declarations_list : 
			...
			if '//' in signal_declaration[0] : 
				...
				# input
				if '[' in signal_declaration[-2] :
					return_string += 'reg ' + signal_declaration[-2] + ' ' + signal_declaration[-1] + ';\n'
				else : 
					return_string += 'reg ' + signal_declaration[-1] + ';\n'
			elif signal_declaration[0] == 'output' : 
				# output
				if '[' in signal_declaration[-2] :
					return_string += 'wire ' + signal_declaration[-2] + ' ' + signal_declaration[-1] + ';\n'
				else : 
		...
		for signal_declaration in self.signal_declarations_list : 
			return_string += '\t\t'
			if self.remove_useless_characters(signal_declaration[0],['\t']).startswith('//') : 
				for element in signal_declaration : 
					return_string += element + ' '
				return_string += '\n'
			else :
